...
import com.recommand.study.util.CommonUtils
 ...
 * @date 2021/01/18
 */
abstract class BaseRecallCF extends RecallCFTrait {
  /**
   * 获取过滤条件
   *
   * @param column 列名
   * @return 过滤条件
   */
  override def getFilterCondition(column: String): String = s"$column <> ${CommonUtils.concatCopy(column)}"
   ...
   * @return DataFrame
   */
  override def getUserMusicFavoriteRate(spark: SparkSession): DataFrame = {
